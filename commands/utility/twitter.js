...
module.exports = {
  name: "twitter",
  description: "Check a twitter profile!",
  category: "utility",
  aliases: [],
  usage: "$twitter <username>",
  run: async (client, msg, args) => {
    let user = args[0]
    if(!user) return msg.channel.send("Please give a valid twitter username!")
    ...
    try {
      ...
      else if(e.status === 404) return msg.channel.send("Username not found")
